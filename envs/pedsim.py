...
import numpy as np
import logging
from utils.action import xy2rscnt, mod2pi, rotate
from utils.state import pack_state
from utils.metrics import get_ttcmd

class Pedsim:
    """
    simulation environment
    """
    def __init__(self, args, rl_mask=None, ppo=None):
        """
        - args: return value of utils.utils.get_args()
        """
        self.meta_data = {'time_unit': args.DT}
        self.ppo = ppo
    ...
    def add_pedestrian(self, position, velocity, destination, direction=None,  init=False):
        """
        ...
        if init:
            ...
            if not self.args.DYNAMIC:
                self.rl_mask =  np.ones(self.num_pedestrians, dtype=bool)
        else:
            ...
            direction_ = float('nan') + torch.zeros((n, self.num_steps or 1, 1))
            direction_[:, -1, :] = torch.atan2(velocity[:, 1], velocity[:, 0]).view(-1, 1).to(self.device)
            arrive_flag_ =  torch.full((n, self.num_steps or 1), False)
            mask_ = torch.full((n, self.num_steps or 1), False)
            mask_[:, -1] = torch.full((n, ), True)
            ...
            self.direction = torch.cat([self.direction, direction_.to(self.device)], dim=0)
            ...
            self.mask = torch.cat([self.mask, mask_.to(self.device)], dim=0)
            self.raw_velocity = self.velocity
            self.num_pedestrians += n
            if not self.args.DYNAMIC:
                self.rl_mask =  np.ones(self.num_pedestrians, dtype=bool)
    ...
    def add_step(self, position, velocity, init=False):
        ...
        if init:
            ...
            self.arrive_flag = torch.cat([self.arrive_flag, torch.full((N, 1), False).to(self.device)], dim=1)
    ...
    def reward(self, index=-1):
        ...
        reward['SMOOTH_V'] = {'SCALE': self.args.RW_SMOOTH_V, 'VALUE': (v - v.clamp(-0.5, 1.5)).abs()}
        reward['SMOOTH_W'] = {'SCALE': self.args.RW_SMOOTH_W, 'VALUE': (w - w.clamp(-np.pi/4, np.pi/4)).abs()}

        # urgency
        reward['URGENCY'] = {'SCALE': self.args.RW_URGENT, 'VALUE': self.mask.sum(dim=-1).to(torch.float32)}
        ...
        if self.num_obstacles > 0:
            ...
            coll_obs = torch.full_like(coll_ped, False)
        coll = coll_ped.any(dim=-1) | coll_obs.any(dim=-1)
        reward['COLL'] = {'SCALE': self.args.RW_COLL, 'VALUE': coll.float()}

        # GAIL
        if self.args.MODEL == 'MAGAIL':
            # State
            s_self, s_int, s_ext = self.get_state(index=-1)
            gen_state = pack_state(s_self, s_int, s_ext)  # (N, 169)
            gen_state[torch.isnan(gen_state)] = 0.0

            # Action
            w0 = self.direction[:,-2, 0]
            w_w0 = self.direction[:, -1, 0]
            v = torch.norm(self.velocity[:, -1, :], dim=-1).view(-1, 1)
            v[torch.isnan(v)] = 0.0
            w =(w_w0 - w0).view(-1, 1)
            w[torch.isnan(w)] = 0.0
            gen_action = torch.cat((v, w), dim=-1)
            gen_r = self.ppo.d(gen_state, gen_action)
            reward['GAIL'] = {'SCALE': self.args.RW_GAIL, 'VALUE': - torch.log(1.0 - torch.clamp(torch.sigmoid(gen_r), 1e-10, 1)).view(-1)}
        ...
        rwd = torch.zeros([self.num_pedestrians, 1], device=self.device)[self.rl_mask]
        for r in reward.values():
            r['VALUE'] = r['VALUE'][self.rl_mask]
    ...
    def action(self, v, w, enable_nan_action=False):
        ...
        w0 = self.direction[self.rl_mask, -1, 0]
        velocity_ = torch.full((self.num_pedestrians, 2), float('nan')).to(self.args.DEVICE)
        velocity_[self.rl_mask, :] = torch.stack([v * torch.cos(w0 + w), v * torch.sin(w0 + w)], dim=-1)

        direction_ = torch.atan2(velocity_[:, 1], velocity_[:, 0])
        self.raw_velocity = torch.cat([self.raw_velocity, velocity_[:, None, :]], dim=1)
        if enable_nan_action:
            # mask = torch.full((self.num_pedestrians, 1), False).to(self.args.DEVICE)
            # mask[self.rl_mask, :] = (~(v.isnan() | w.isnan()).view(-1, 1))
            mask = self.mask[:, (-1,)]
            mask[self.rl_mask, :] = (~(v.isnan() | w.isnan()).view(-1, 1))
        else:
            mask = self.mask[:, (-1,)]
        ...
        arrive_flag_ = torch.where(self.mask[:, -1], torch.norm(self.position[:, -1, :] - self.destination, dim=-1) 
                                   < self.ped_radius, self.arrive_flag[:, -1])
        self.arrive_flag = torch.cat([self.arrive_flag, arrive_flag_.view(-1, 1)], dim=1)

        # update dynamic model
        if self.args.DYNAMIC:
            ppo_step = [position_, velocity_, arrive_flag_, mask, direction_]
            position_d, velocity_d, arrive_flag_d, mask_d, direction_d = self.dynamic_model(rl_mask=self.rl_mask, ppo_step=ppo_step)
            self.position[~self.rl_mask, -1, :]  = position_d
            self.velocity[~self.rl_mask, -1, :] = velocity_d
            self.arrive_flag[~self.rl_mask, -1] = arrive_flag_d
            self.mask[~self.rl_mask, -1] = mask_d
            self.direction[~self.rl_mask, -1, :] = direction_d

            # rerun = True if torch.isnan(position_d[mask_d]).any() else False
    ...
    def get_state(self, index=-1, include_radius=False, max_obstacles_num=10):
        ...
        s_des = xy2rscnt(pos=self.destination[:, :] - self.position[:, index, :], 
