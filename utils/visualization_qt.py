...
from utils.action import rotate
...
class Visualization:
    ...
    def init_ctrl(self):
        ...
        self.ctrl_widget['back'] = QtWidgets.QPushButton('Back')
        self.ctrl_widget['back'].clicked.connect(self._action_backward)
        w1.addWidget(self.ctrl_widget['back'])

        self.ctrl_widget['pause'] = QtWidgets.QPushButton('Pause/Play')
        ...
        self.ctrl_widget['step'] = QtWidgets.QPushButton('Step')
        ...
        self.ctrl_widget['focus'] = Parameter(name='Focus ID: ', type='int', value=-1)  # 当前关注的行人
        ...
        self.ctrl_widget['direc'] = Parameter(name='Show Direction: ', type='bool', value=True)
        pw.addParameters(self.ctrl_widget['direc'])

        self.ctrl_widget['label'] = Parameter(name='Pedestrian ID: ', type='bool', value=True)
        pw.addParameters(self.ctrl_widget['label'])

        self.ctrl_widget['trace'] = Parameter(name='Show Trace: ', type='bool', value=False)
        pw.addParameters(self.ctrl_widget['trace'])

        self.ctrl_widget['des'] = Parameter(name='Show Destination: ', type='bool', value=False)
        pw.addParameters(self.ctrl_widget['des'])

        self.ctrl_widget['ray'] = Parameter(name='Show Attention: ', type='bool', value=False)
        pw.addParameters(self.ctrl_widget['ray'])

        self.ctrl_widget['conflict'] = Parameter(name='Show Collision: ', type='bool', value=False and hasattr(self.env, 'CAP_flag'))
        pw.addParameters(self.ctrl_widget['conflict'])

        self.ctrl_widget['predict'] = Parameter(name='Show Prediction: ', type='bool', value=False)
    ...
    def update(self):
        ...
        # 场景信息
        info = dict(
            collision = calc_collision(self.env),
            # displacement = calc_displacement(env_imit=self.env),
            # deviation = calc_deviation(env_imit=self.env),
            # velocityLocomotion = calc_v_locomotion(env_imit=self.env),
            # angleLocomotion = calc_a_locomotion(env_imit=self.env),
            # energy = calc_energy(env_imit=self.env),
            # steerEnergy = calc_steer_energy(env_imit=self.env)
        )
