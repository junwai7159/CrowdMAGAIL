...
class PedState:
    ...
    def state(self, state):
        ...
        if self.initial_speeds is None:
            self.initial_speeds = self.speeds(self.state)
        if self.initial_speeds.shape[0] < state.shape[0]:
            n = state.shape[0] - self.initial_speeds.shape[0]
            self.initial_speeds = np.append(self.initial_speeds, np.full(n, 1.0))
        self.max_speeds = self.max_speed_multiplier * self.initial_speeds

        self.ped_states.append(self._state.copy())

    def get_states(self):
        tmp_n = [self.ped_states[i].shape[0] for i in range(len(self.ped_states))]
        max_n = max(tmp_n)
        n = max_n  - min(tmp_n)

        if n > 0:
            for i in range(len(self.ped_states)):
                if self.ped_states[i].shape[0] != max_n:
                    self.ped_states[i] = np.pad(self.ped_states[i], ((0, n), (0, 0)), mode='constant', constant_values=float('nan'))
    ...
    def speeds(self, state):
        ...
        return stateutils.speeds(state)
